...
import logging
import time
from io import StringIO  # ✅ For parsing CSV response
...
def fetch_urlhaus():
    ...
    if response:
        try:
            data = pd.read_csv(StringIO(response.text), comment='#', low_memory=False)
            print(f"[DEBUG] URLhaus columns: {list(data.columns)}")

            # Ensure the 'url' column exists or find a suitable column
            if 'url' in data.columns:
                urls = [{"phishing_url": row['url'], "source": "URLhaus"} for _, row in data.iterrows()]
            else:
                # Auto-detect URL-like column
                possible_cols = [col for col in data.columns if 'url' in col.lower()]
                if possible_cols:
                    col_name = possible_cols[0]
                    print(f"[INFO] Using detected column: {col_name}")
                    urls = [{"phishing_url": row[col_name], "source": "URLhaus"} for _, row in data.iterrows()]
                else:
                    print("[ERROR] No 'url' column found in URLhaus data.")
                    return []
