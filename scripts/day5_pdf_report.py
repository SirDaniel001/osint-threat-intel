...
# Fonts (DejaVu for Unicode safety)
...
def generate_pdf(total_records, unique_domains, kenyan_count, tld_chart, domains_chart):
    ...
    pdf.cell(0, 10, "OSINT Threat Intelligence - Day 5 Report",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')

    # Date
    pdf.set_font("DejaVu", '', 12)
    date_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    pdf.cell(0, 10, f"Generated on: {date_str}",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    ...
    pdf.cell(60, 10, "Value", border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(60, 10, "Total Records", border=1)
    pdf.cell(60, 10, str(total_records), border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(60, 10, "Unique Domains", border=1)
    pdf.cell(60, 10, str(unique_domains), border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(60, 10, "Kenyan URLs", border=1)
    pdf.cell(60, 10, str(kenyan_count), border=1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)

    # Charts
    pdf.ln(10)
    pdf.set_font("DejaVu", 'B', 14)
    pdf.cell(0, 10, "TLD Distribution", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.image(tld_chart, x=10, w=180)

    pdf.ln(80)
    pdf.cell(0, 10, "Top 5 Domains", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.image(domains_chart, x=10, w=180)
